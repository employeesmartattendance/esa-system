<template>
  <div class="esa-website" :class="{ dark: isDark }">
    ...
    <!-- ══════════ WHATSAPP FLOATING BUTTON (website page only) ══════════ -->
    <a
      :href="whatsappHref"
      target="_blank"
      rel="noopener noreferrer"
      class="website-wa-fab"
      aria-label="Chat with us on WhatsApp"
      title="Chat with us on WhatsApp"
    >
      <span class="website-wa-pulse"></span>
...
<style scoped>
/* ═══════════════════════════════════
   WHATSAPP FLOATING BUTTON (website only)
═══════════════════════════════════ */
.website-wa-fab {
  position: fixed;
...
.website-wa-fab:hover {
  transform: translateY(-2px) scale(1.05);
  box-shadow: 0 10px 30px rgba(37, 211, 102, 0.6);
}
.website-wa-fab:active { transform: scale(0.96); }
.website-wa-pulse {
  position: absolute;
  inset: 0;
  border-radius: 50%;
...
/* Skip render/paint work for sections until they're near the viewport —
   this is what keeps scrolling smooth on low-end/mobile devices. The
   contain-intrinsic-size estimate prevents layout jump when a section
   switches from "skipped" to "rendered" as it scrolls into view. */
.section-pad {
  padding:96px 0;
