  ...
  <div>
    ...
    <div class="grid-2" style="margin-bottom: 24px">
        ...
        <div class="donut-wrap">
          <svg class="donut-svg" viewBox="0 0 120 120">
            ...
            <circle
              ...
              :stroke-dashoffset="314 - (314 * attendanceRate) / 100"
      ...
      <div class="glass section-card">
        <div class="card-header">
          <div class="card-title">
            <AppIcon name="school" :size="18" color="var(--primary)" />Schools
            Overview
          </div>
        ...
        <div v-if="schools.length" class="school-list">
          <div
            v-for="sch in schools.slice(0, 2)"
            :key="sch.id"
            class="school-item"
          >
            ...
            <div class="school-item-right">
              <div class="sch-present">
                {{ sch.present_today ?? 0 }} present
              </div>
              <AppBadge
                :variant="sch.status === 'active' ? 'active' : 'inactive'"
                :label="sch.status"
                dot
              />
            </div>
        ...
        <EmptyState
          ...
          icon="school"
          title="No schools"
          message="Create a school to see it here"
        />
      </div>
    </div>
    ...
    <div class="grid-2">
      <div class="glass section-card">
        <div class="card-header">
          <div class="card-title">
            <AppIcon name="activity" :size="18" color="var(--primary)" />Recent
            Activity
          </div>
        </div>
          ...
          <div v-for="log in activity" :key="log.id" class="activity-item">
            ...
            <div class="activity-content">
              ...
              <div class="activity-meta">
                {{ log.user_name || "System" }} · {{ timeAgo(log.timestamp) }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="glass section-card">
        <div class="card-header">
          ...
          <span class="status-live" :style="allOk ? '' : 'color:var(--danger)'">
            <span
              class="pulse-dot"
              :style="allOk ? '' : 'background:var(--danger)'"
            ></span>
        ...
        <div class="system-checks">
          <div class="sys-check" v-for="c in systemChecks" :key="c.label">
            ...
            <AppBadge
              v-else
              :variant="c.ok ? 'active' : 'danger'"
              :label="c.ok ? 'Online' : 'Offline'"
              dot
            />
          </div>
        </div>
        <div class="quick-actions">
          <div class="card-header" style="margin-top: 16px">
            <div class="card-title">
              <AppIcon name="activity" :size="16" color="var(--primary)" />Quick
              Actions
            </div>
          </div>
          <div class="qa-grid">
            <button class="qa-btn" @click="$emit('go-schools')">
              <AppIcon name="plus" :size="16" color="var(--primary)" />New
              School
            </button>
