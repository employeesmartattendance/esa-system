    ...
    <div v-else class="ts-grid">
      <div class="glass ts-card" v-for="s in schools" :key="s.id">
        ...
        <div class="ts-info">
          ...
          <div class="ts-meta">
            <span class="ts-badge-status" :class="s.is_active ? 'active' : 'inactive'">
    ...
    >
      <div style="display:flex;flex-direction:column;gap:18px">
        ...
        <div class="form-group">
          ...
          >
            ...
            <div v-if="previewSrc" class="drop-preview">
              ...
              <div class="drop-preview-info">
                <span class="drop-filename">{{ selectedFile?.name || 'Current logo' }}</span>
                <button type="button" class="drop-remove" @click.stop="clearFile">
          ...
          <span v-if="uploadError" class="form-hint" style="color:var(--danger)">{{ uploadError }}</span>
        </div>

        <!-- Live preview -->
        <div class="logo-preview glass">
          <span class="form-hint" style="margin-bottom:10px;display:block">Live Preview</span>
          <div class="preview-badge">
            ...
            />
            <div v-else-if="form.name" class="ts-initial" style="font-size:22px">
              {{ initials(form.name) }}
            </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input class="form-input" type="number" v-model.number="form.sort_order" min="0" placeholder="0" />
      ...
      <div v-if="deleteTarget" style="display:flex;flex-direction:column;gap:20px">
        <div class="delete-confirm-body">
          ...
          <div class="delete-school-preview">
            ...
            />
            <div v-else class="delete-school-initial">{{ initials(deleteTarget.name) }}</div>
          </div>
          <div>
            <p style="font-size:15px;font-weight:700;margin-bottom:6px">{{ deleteTarget.name }}</p>
            <p style="font-size:14px;color:var(--text-secondary);line-height:1.6">
              This school badge will be removed from the ESA website. This action cannot be undone.
            </p>
          </div>
        </div>
        ...
        <div class="modal-footer-btns">
          ...
          <button
            ...
            @click="doDelete"
          >
            <div v-if="deleting" class="spinner" style="width:14px;height:14px;border-color:rgba(255,255,255,.3);border-top-color:#fff"></div>
...
.info-banner {
  ...
  color:var(--text-secondary); line-height:1.65; border-left:3px solid var(--accent);
}

/* ── Cards grid ─────────────────────────────────────────────────────────── */
.ts-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(280px,1fr)); gap:16px; }
.ts-card { display:flex; align-items:center; gap:14px; padding:18px 20px; border-radius:var(--radius-lg); transition:all var(--transition); }
.ts-card:hover { transform:translateY(-2px); box-shadow:var(--card-shadow-lg); }

.ts-badge-preview {
  width:64px; height:48px; border-radius:var(--radius-sm); flex-shrink:0;
  display:flex; align-items:center; justify-content:center;
  background:var(--bg-secondary); border:1px solid var(--surface-border); overflow:hidden;
}
.ts-logo    { max-width:56px; max-height:40px; object-fit:contain; display:block; }
.ts-initial { font-size:20px; font-weight:800; color:var(--primary); }

.ts-info  { flex:1; min-width:0; }
.ts-name  { font-size:14px; font-weight:700; margin-bottom:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.ts-meta  { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
.ts-badge-status { font-size:10px; font-weight:700; padding:2px 8px; border-radius:99px; text-transform:uppercase; }
.ts-badge-status.active   { background:rgba(16,185,129,0.15); color:var(--success); }
.ts-badge-status.inactive { background:rgba(239,68,68,0.1);   color:var(--danger);  }
.ts-order { font-size:11px; color:var(--text-muted); }
...
.btn-danger-soft {
  ...
  padding:6px 10px; cursor:pointer; transition:all var(--transition); gap:4px;
  font-family:var(--font);
}
.btn-danger-soft:hover { background:var(--danger); color:#fff; }

.ts-empty { display:flex; flex-direction:column; align-items:center; gap:14px; padding:64px 32px; text-align:center; border-radius:var(--radius-xl); }

/* ── Drop zone ──────────────────────────────────────────────────────────── */
.drop-zone {
  border:2px dashed var(--surface-border); border-radius:var(--radius);
  background:var(--bg); cursor:pointer; transition:all var(--transition);
  min-height:110px; display:flex; align-items:center; justify-content:center;
}
.drop-zone:hover, .drop-zone.drop-over { border-color:var(--primary); background:rgba(37,99,235,0.04); }
.drop-zone.has-file { border-style:solid; border-color:var(--success); background:rgba(16,185,129,0.04); }

.drop-placeholder { display:flex; align-items:center; gap:16px; padding:20px; width:100%; }
.drop-icon { width:52px; height:52px; background:rgba(37,99,235,0.1); border-radius:var(--radius); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
.drop-text { display:flex; flex-direction:column; gap:3px; }
.drop-text strong { font-size:14px; color:var(--text); }
.drop-text span   { font-size:12px; color:var(--text-muted); }

.drop-preview { display:flex; align-items:center; gap:14px; padding:16px; width:100%; }
.drop-img  { width:60px; height:44px; object-fit:contain; border-radius:var(--radius-sm); border:1px solid var(--surface-border); }
.drop-preview-info { display:flex; flex-direction:column; gap:6px; flex:1; min-width:0; }
.drop-filename { font-size:13px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--text); }
.drop-remove { display:inline-flex; align-items:center; gap:4px; font-size:12px; font-weight:600; color:var(--danger); background:transparent; border:none; cursor:pointer; padding:0; font-family:var(--font); }
.drop-remove:hover { text-decoration:underline; }

.upload-progress { height:4px; background:var(--bg-secondary); border-radius:99px; overflow:hidden; margin-top:6px; }
.upload-bar { height:100%; background:linear-gradient(90deg,var(--primary),var(--accent)); border-radius:99px; transition:width .3s ease; }
...
.logo-preview { padding:16px; border-radius:var(--radius); display:flex; flex-direction:column; align-items:center; }
.preview-badge { width:110px; height:68px; border-radius:var(--radius-sm); background:var(--bg-secondary); border:1px solid var(--surface-border); display:flex; align-items:center; justify-content:center; overflow:hidden; }

.toggle-wrap   { display:flex; align-items:center; gap:12px; margin-top:6px; }
.toggle-switch { position:relative; width:44px; height:24px; flex-shrink:0; display:block; }
.toggle-switch input { opacity:0; width:0; height:0; position:absolute; }
.toggle-slider { position:absolute; top:0; left:0; right:0; bottom:0; background:var(--text-muted); border-radius:99px; transition:.3s; cursor:pointer; }
.toggle-slider::before { content:''; position:absolute; width:18px; height:18px; left:3px; bottom:3px; background:#fff; border-radius:50%; transition:.3s; }
input:checked + .toggle-slider { background:var(--primary); }
input:checked + .toggle-slider::before { transform:translateX(20px); }

.modal-footer-btns { display:flex; gap:10px; padding-top:4px; }

/* ── Delete confirmation ─────────────────────────────────────────────────── */
.delete-confirm-body { display:flex; align-items:center; gap:16px; padding:8px 0; }
.delete-school-preview {
  width:72px; height:56px; border-radius:var(--radius-sm); flex-shrink:0;
  display:flex; align-items:center; justify-content:center;
  background:var(--bg-secondary); border:1px solid var(--surface-border); overflow:hidden;
}
.delete-school-img     { max-width:60px; max-height:44px; object-fit:contain; display:block; }
.delete-school-initial { font-size:22px; font-weight:800; color:var(--primary); }

/* ── Spinner ─────────────────────────────────────────────────────────────── */
.spinner { border:2px solid rgba(37,99,235,.3); border-top-color:var(--primary); border-radius:50%; animation:spin .7s linear infinite; }
...
@media (max-width:600px) {
  ...
  .delete-confirm-body { flex-direction:column; text-align:center; }
  .ts-badge-preview { display: none !important; }
}
</style>
