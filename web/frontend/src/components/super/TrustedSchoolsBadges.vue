 ...
 * In dev, the Vite proxy maps /uploads → esasystem.onrender.com/uploads, so relative paths work directly.
 * We keep this passthrough for the dashboard (proxy handles it).
 */
function toDisplayUrl(url) {
  if (!url) return "";
  if (url.startsWith("http") || url.startsWith("//")) return url;
  // Relative /uploads/... path — served via Vite proxy to esasystem.onrender.com in dev
  return url;
}
