...
// socket.js pulls in socket.io-client — load it lazily (only for logged-in
// sessions) so public pages like /website don't pay for that bundle weight.
...
const bannerText = computed(() => {
  ...
  if (!isOnline.value) {
    ...
    const queue = pendingCount.value > 0 ? ` · ${pendingCount.value} queued` : ''
    if (isElectron) {
      return age
        ? `You are offline. Showing last synced data. Last sync ${age}${queue}`
        : `You are offline. Showing last synced data.${queue}`
    }
    return age ? `Offline — Last synced ${age}${queue}` : 'Offline — No cached data'
  }
  return ''
})
...
async function setupForceLogoutListener() {
  ...
  socket.on('force_logout', (data) => {
    forceLogoutMessage.value = data?.message || 'Your school has been deactivated.'
...
function checkInitialAuth() {
  if (auth.isLoggedIn && isElectron) {
    ...
    const apiUrl = localStorage.getItem('esa_backend_url') || 'https://esa-system.onrender.com/api'
...
.banner-icon  { font-size: 15px; line-height: 1; }
...
.banner-sync-btn:hover { background: rgba(255,255,255,0.15); }
...
.flm-icon { font-size: 52px; margin-bottom: 16px; color: #ef4444; font-weight: bold; }
.force-logout-modal h2 { color: #ef4444; font-size: 22px; font-weight: 800; margin: 0 0 12px; }
.force-logout-modal p  { color: #94a3b8; font-size: 15px; line-height: 1.6; margin: 0 0 28px; }
.flm-btn {
...
.flm-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(239,68,68,0.4); }
