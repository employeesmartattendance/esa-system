...
export const useAuthStore = defineStore('auth', () => {
  ...
  const daysRemaining = computed(() => {
    ...
    return Math.max(0, Math.ceil((TOKEN_LIFETIME_MS - elapsed) / (24 * 60 * 60 * 1000)))
  })
  ...
  async function login(email, password) {
    ...
    try {
      ...
      const payload = res?.token
        ? res
        : (res?.data?.token ? res.data : (res?.data || null))
  ...
  async function fetchMe() {
    try {
      ...
      user.value = (res && typeof res === 'object' && !Array.isArray(res)) ? res : (res?.data ?? res)
      localStorage.setItem('esa_user', JSON.stringify(user.value))
    } catch (_) { _clearSession() }
  }
